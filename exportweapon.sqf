...
if (_exportscript == 1) exitwith {player groupchat "you are already exporting please wait"}; //Checks that your not already exporting
...
if (_exportscript == 1) exitwith {player groupchat "you are already exporting please wait"};	 //Checks again

_exportscript = 1; // The script starts now.

if(_stock >= _maxstock)exitwith{player groupchat "the shop has reached its maximum stock for this item";_exportscript = 0};

sleep 1;
																			
['geld', _preis] call INV_AddInventoryItem;

_exportscript = 0; // End of script
