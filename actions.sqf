...
//======================================Milking========================================================
_role addaction ["Milk the cow","noscript.sqf",'_no = ceil(random 7);if(milking)exitwith{};milking=true; if (((call INV_GetOwnWeight) + ("milk" call INV_getitemTypeKg)*_no) > INV_Tragfaehigkeit)exitwith{player groupChat localize "STRS_inv_buyitems_maxgewicht"}; ["milk", _no] call INV_AddInventoryItem;player switchmove "repairingkneel"; nearestobjects [getpos player, ["cow04"], 3] select 0 disableAI "move"; sleep 18 ; player switchmove ","; nearestobjects [getpos player, ["cow04"], 3] select 0 enableAI "move" ;milking=false; player groupchat format ["you got %1 milk",_no];',1,true,true,"",'player distance (nearestobjects [getpos player, ["cow04"], 3] select 0) < 2 and isciv'];
//===================================== TARGET RESET ==================================================
