...
//======================================Milking========================================================
_role addaction ["Milk the cow","noscript.sqf",'_no = ceil(random 7);if(milking)exitwith{};milking=true; if (((call INV_GetOwnWeight) + ("milk" call INV_getitemTypeKg)*_no) > INV_Tragfaehigkeit)exitwith{player groupChat localize "STRS_inv_buyitems_maxgewicht";milking=false;}; ["milk", _no] call INV_AddInventoryItem;player playmove "AinvPknlMstpSlayWrflDnon_medic"; nearestobjects [getpos player, ["cow04"], 3] select 0 disableAI "move"; sleep 5;waituntil {animationstate player != "AinvPknlMstpSlayWrflDnon_medic"}; nearestobjects [getpos player, ["cow04"], 3] select 0 enableAI "move" ;milking=false; player groupchat format ["you got %1 milk",_no];',1,true,true,"",'player distance (nearestobjects [getpos player, ["cow04"], 3] select 0) < 2 and isciv'];
//===================================== TARGET RESET ==================================================
...
_role addaction ["Get patrol mission","coppatrol.sqf",["start"],1,false,true,"","iscop and player distance copcar2 <= 4 and !pmissionactive and !patrolwaittime"];
_role addaction ["Cancel patrol mission","coppatrol.sqf",["end"],1,false,true,"","iscop and pmissionactive and player distance copcar2 <= 4"];
//================================== COP DELETE EVIDENCE ================================================
...
_role addaction ["Impound vehicle","noscript.sqf",'_vcl = (nearestobjects [getpos player, ["Air", "Ship", "LandVehicle"], 3] select 0);[_vcl, "impound"] execVM "impound.sqf";',1,true,true,"",'_vcl = (nearestobjects [getpos player, ["Air", "Ship", "LandVehicle"], 3] select 0);player distance _vcl < 20 and _vcl in INV_ServerVclArray and iscop'];

_role addaction ["Pull out","noscript.sqf",'(nearestobjects [getpos player, ["Air", "Ship", "LandVehicle"], 3] select 0) execVM "pullout.sqf";',1,true,true,"",'_vcl = (nearestobjects [getpos player, ["Air", "Ship", "LandVehicle"], 3] select 0);player distance _vcl < 10 and count (crew _vcl) > 0 and _vcl in INV_ServerVclArray and (call INV_isArmed)'];
...
_role addaction ["Industrial Merchant","shopdialogs.sqf",[_num1],1,false,true,"","player distance shop1export <= 3"];
_role addaction ["Swonto Merchant","shopdialogs.sqf",[_num2],1,false,true,"","player distance shop2export <= 3"];
_role addaction ["Kingala Merchant","shopdialogs.sqf",[_num3],1,false,true,"","player distance shop3export <= 3"];
_role addaction ["Kinsella Merchant","shopdialogs.sqf",[_num4],1,false,true,"","player distance shop4export <= 3"];
_role addaction ["Nubak Merchant","shopdialogs.sqf",[_num5],1,false,true,"","player distance shop5export <= 3"];
_role addaction ["Bolabongo Merchant","shopdialogs.sqf",[_num6],1,false,true,"","player distance shop6export <= 3"];
//======================================= GANG GUNSHOPS ==================================================
...
//================================REFRESH SERVER SIDE SCRIPTS ===========================================
_role addaction ["Refesh Server Side Scripts","resetserver.sqf",[reset],1,false,true,"","player distance refreshscripts <= 5"];
